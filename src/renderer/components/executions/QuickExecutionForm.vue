...
const localAmountMinor = computed(() => localPriceMinor.value * Number(form.quantity || 0))
...
<template>
  <form class="quick-form" @submit.prevent="submit">
    ...
    <div class="quick-grid quick-grid--primary">
      ...
      <label class="field">
        <span>成交均价</span>
        <input v-model.number="form.localPrice" aria-label="成交均价" type="number" min="0.01" step="0.01" required />
    ...
    <details class="optional-details">
      ...
      <div class="quick-grid optional-grid">
        ...
        <label class="field"><span>成交时间</span><input v-model="form.executedAt" type="datetime-local" /></label>
...
@media (max-width: 900px) { .quick-grid--primary { grid-template-columns: repeat(2, minmax(0, 1fr)); }.amount-row { grid-template-columns: 1fr; } }
</style>

...
const plans = [
  { id: 'expired', status: 'qualified' as const, ruleVersionId: 'rule-1', draft: { code: '0700.HK', quantity: 100, validUntil: '2020-01-01T00:00:00.000Z' }, validation: { savable: true, qualified: true, findings: [], metrics: {} } },
  { id: 'draft', status: 'draft' as const, ruleVersionId: 'rule-1', draft: { code: '0700.HK', quantity: 100, validUntil: '2099-01-01T00:00:00.000Z' }, validation: { savable: true, qualified: false, findings: [], metrics: {} } },
  { id: 'ok', status: 'qualified' as const, ruleVersionId: 'rule-1', draft: { code: '0700.HK', quantity: 100, validUntil: '2099-01-01T00:00:00.000Z' }, validation: { savable: true, qualified: true, findings: [], metrics: {} } },
]
...
describe('ExecutionForm', () => {
  it('lists only qualified and unexpired plans and records emotion', async () => {
    ...
    const planSelect = screen.getByLabelText('对应计划（可不选）') as HTMLSelectElement
    ...
    expect(submissions[0]?.[0]).toEqual(expect.objectContaining({
      ...
      emotion: { fearScore: 2, greedScore: 0, revengeScore: 0 },
    }))
  })
})

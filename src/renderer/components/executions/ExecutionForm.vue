...
const props = defineProps<{ instruments: Instrument[]; plans: PlanRecord[]; busy: boolean }>()
...
<template>
  <form class="form-stack" @submit.prevent="submit">
    ...
    <label class="field"><span>对应计划（可不选）</span><select v-model="form.planId"><option value="">无计划成交</option><option v-for="plan in plans" :key="plan.id" :value="plan.id">{{ plan.draft.code }} · {{ plan.status }} · {{ plan.draft.quantity }} 股</option></select></label>

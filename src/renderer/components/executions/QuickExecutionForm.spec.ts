...
describe('QuickExecutionForm', () => {
  ...
  it('applies screenshot facts and keeps ETF price precision with user-entered emotion', async () => {
    ...
    expect(submissions[0]?.[0]).toEqual(expect.objectContaining({
      ...
      emotion: { state: 'recorded', fearScore: 3, greedScore: 1, revengeScore: 0 },
    }))
  })

  it('preserves an explicit unknown emotion instead of treating it as a zero rating', async () => {
    const rendered = render(QuickExecutionForm, { props: { instruments: [cnyInstrument], plans: [], busy: false } })
    await fireEvent.update(screen.getByLabelText('成交均价'), '4.2')
    await fireEvent.click(screen.getByLabelText('记不清'))
    await confirmFacts()
    await fireEvent.click(screen.getByRole('button', { name: '立即如实入账' }))

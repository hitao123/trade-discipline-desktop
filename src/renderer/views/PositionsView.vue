...
const { data, loading, error, refresh } = useRemoteData((signal) => api.request<Portfolio>('/api/portfolio', { signal }))
const positions = computed(() => Object.values(data.value?.positions ?? {}))
const focusedAlertID = computed(() => typeof route.query.alert === 'string' ? route.query.alert : '')
const orderedAlerts = computed(() => {
  const id = focusedAlertID.value
  if (!id)
    return alerts.value
  return [...alerts.value].sort((left, right) => Number(right.id === id) - Number(left.id === id))
})
...
<template>
  <div>
    ...
    <section v-if="orderedAlerts.length" class="review-list" aria-label="待完成持仓复核"><PositionReviewPanel v-for="alert in orderedAlerts" :key="alert.id" :alert="alert" :busy="reviewingAlertID === alert.id" :error="reviewingAlertID === alert.id ? monitorError : ''" @submit="submitReview" /></section>
    <div v-if="data" class="portfolio-summary"><div><span>可用现金</span><strong>{{ formatCNY(data.availableCashFen) }}</strong></div><div><span>累计亏损（含浮亏）</span><strong>{{ formatCNY(data.cumulativeLossFen) }}</strong></div></div>
    <div class="table-frame">
      <table><thead><tr><th>证券</th><th>数量</th><th>最新收盘</th><th>人民币成本</th><th>人民币市值</th><th>浮动盈亏</th><th>已实现盈亏</th><th>风险标签</th><th>操作</th></tr></thead><tbody>
        <tr v-for="position in positions" :key="position.instrumentId">
          ...
          <td><template v-if="position.referencePriceMinor">{{ formatPrice(position.referencePriceMinor, position.currency || 'CNY') }}</template><span v-else class="muted">待同步</span></td>
          ...
          <td>{{ position.market }} · {{ position.currency }}</td>

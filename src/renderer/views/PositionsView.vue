...
import type { Portfolio } from '@/renderer/types'
...
<template>
  <div>
    <PageHeader eyebrow="POSITIONS" title="持仓由成交重放，不可直接改写" description="数量、人民币成本和已实现盈亏来自不可变事件。录错时追加冲正，旧记录仍留在审计中。"><button class="button" type="button" :disabled="loading" @click="refresh">重新计算</button></PageHeader>
...
.table-frame { overflow: auto; border: 1px solid var(--line); } table { width: 100%; min-width: 980px; border-collapse: collapse; font-size: 13px; } th { padding: 12px 14px; color: var(--ink-faint); background: var(--paper-deep); font-size: 10px; text-align: left; } td { padding: 15px 14px; border-top: 1px solid var(--line); }.negative { color: var(--accent); }.positive { color: var(--success); }.muted { color: var(--ink-faint); }.empty-cell { padding: 44px; color: var(--ink-faint); text-align: center; }
</style>

...
func (s *Store) HasAllocation(ctx context.Context) (bool, error) {
	var exists int
	if err := s.db.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM allocation_profiles)`).Scan(&exists); err != nil {
		return false, fmt.Errorf("inspect allocation profile: %w", err)
	}
	return exists == 1, nil
}
...
func (s *Store) CreateAllocationProfile(ctx context.Context, draft domain.AllocationDraft, reason string, now time.Time) (domain.AllocationVersion, error) {
	...
	if err != nil {
		return domain.AllocationVersion{}, fmt.Errorf("encode initial allocation: %w", err)
	}
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return domain.AllocationVersion{}, fmt.Errorf("begin allocation profile: %w", err)
	}
	defer func() { _ = tx.Rollback() }()
	var count int
	if err := tx.QueryRowContext(ctx, `SELECT count(*) FROM allocation_profiles`).Scan(&count); err != nil {
		return domain.AllocationVersion{}, err
	}
	if count != 0 {
		return domain.AllocationVersion{}, fmt.Errorf("资产配置已经建立")
	}
	stamp := now.UTC().Format(time.RFC3339Nano)
	version := domain.AllocationVersion{ID: NewID("allocation-version"), ProfileID: allocationProfileID, Version: 1, Draft: draft, Reason: reason, CreatedAt: now.UTC()}
	if _, err := tx.ExecContext(ctx, `INSERT INTO allocation_profiles(id, created_at) VALUES(?,?)`, allocationProfileID, stamp); err != nil {
		return domain.AllocationVersion{}, fmt.Errorf("insert allocation profile: %w", err)
	}
	if _, err := tx.ExecContext(ctx, `INSERT INTO allocation_versions(id, profile_id, version, draft_json, change_reason, created_at, previous_id) VALUES(?,?,?,?,?,?,NULL)`, version.ID, version.ProfileID, 1, string(raw), reason, stamp); err != nil {
		return domain.AllocationVersion{}, fmt.Errorf("insert allocation version: %w", err)
	}
	if err := appendAudit(ctx, tx, "allocation_profile", allocationProfileID, "created", nil, string(raw), now); err != nil {
		return domain.AllocationVersion{}, err
	}
	if err := tx.Commit(); err != nil {
		return domain.AllocationVersion{}, fmt.Errorf("commit allocation profile: %w", err)
	}
	return version, nil
}
...
func (s *Store) AppendAllocationAdjustment(ctx context.Context, profileID string, event domain.AllocationAdjustmentEvent, now time.Time) (domain.AllocationAdjustmentEvent, error) {
	if event.Source != "manual_adjustment" {
		return domain.AllocationAdjustmentEvent{}, fmt.Errorf("配置调整来源无效")
	}
	if event.ObservedAt.IsZero() {
		return domain.AllocationAdjustmentEvent{}, fmt.Errorf("配置调整时间不能为空")
	}
	event.ID = NewID("allocation-adjustment")
	event.ProfileID = profileID
	event.ObservedAt = event.ObservedAt.UTC()
	event.CreatedAt = now.UTC()
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return domain.AllocationAdjustmentEvent{}, fmt.Errorf("begin allocation adjustment: %w", err)
	}
	defer func() { _ = tx.Rollback() }()
	if _, err := tx.ExecContext(ctx, `INSERT INTO allocation_adjustment_events(id, profile_id, item_key, adjustment_fen, source, observed_at, created_at) VALUES(?,?,?,?,?,?,?)`, event.ID, event.ProfileID, event.ItemKey, event.AdjustmentFen, event.Source, event.ObservedAt.Format(time.RFC3339Nano), event.CreatedAt.Format(time.RFC3339Nano)); err != nil {
		return domain.AllocationAdjustmentEvent{}, fmt.Errorf("insert allocation adjustment: %w", err)
	}
	after, _ := json.Marshal(map[string]any{"itemKey": event.ItemKey, "adjustmentFen": event.AdjustmentFen, "observedAt": event.ObservedAt})
	if err := appendAudit(ctx, tx, "allocation_adjustment", event.ID, "recorded", nil, string(after), now); err != nil {
		return domain.AllocationAdjustmentEvent{}, err
	}
	if err := tx.Commit(); err != nil {
		return domain.AllocationAdjustmentEvent{}, fmt.Errorf("commit allocation adjustment: %w", err)
	}
	return event, nil
}

func (s *Store) LatestAllocationAdjustments(ctx context.Context, profileID string) (map[string]domain.AllocationAdjustmentEvent, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, profile_id, item_key, adjustment_fen, source, observed_at, created_at FROM allocation_adjustment_events WHERE profile_id=? ORDER BY item_key, observed_at DESC, created_at DESC, rowid DESC`, profileID)
	if err != nil {
		return nil, fmt.Errorf("list latest allocation adjustments: %w", err)
	}
	defer rows.Close()
	adjustments := make(map[string]domain.AllocationAdjustmentEvent)
	for rows.Next() {
		var event domain.AllocationAdjustmentEvent
		var observed, created string
		if err := rows.Scan(&event.ID, &event.ProfileID, &event.ItemKey, &event.AdjustmentFen, &event.Source, &observed, &created); err != nil {
			return nil, fmt.Errorf("scan allocation adjustment: %w", err)
		}
		if _, exists := adjustments[event.ItemKey]; exists {
			continue
		}
		event.ObservedAt, _ = time.Parse(time.RFC3339Nano, observed)
		event.CreatedAt, _ = time.Parse(time.RFC3339Nano, created)
		adjustments[event.ItemKey] = event
	}
...
func initialAllocationDraft() domain.AllocationDraft {
	return domain.AllocationDraft{
		...
		Items: []domain.AllocationItem{
			{Key: "tencent", Name: "腾讯", TargetWeightBP: 3_900, TargetShares: 200, InstrumentCodes: []string{"0700.HK"}, BuyRule: "分批到 200 股；若一路上涨，不为凑股数追高。", SellRule: "主要按仓位再平衡和基本面变化卖；游戏、广告、微信生态与自由现金流持续恶化，或 AI 投入长期无法转化时重新评估。", Note: "核心成长；计划最终约 200 股。"},
			...
			{Key: "semiconductor-equipment-etf-159558", Name: "半导体设备 ETF 159558", TargetWeightBP: 1_250, InstrumentCodes: []string{"159558"}, BuyRule: "明显回撤且基本面未坏时分档加仓；更深回撤或极端杀估值时再加。", SellRule: "估值过热或产业资本开支、龙头订单增速和盈利预期系统性转弱时分批减仓。", Note: "已有 1 万元；达到目标仓后不机械加仓。"},
			...
			{Key: "communication-etf", Name: "通信 ETF", TargetWeightBP: 600, InstrumentCodes: []string{"515880"}, BuyRule: "先观察，完成 ETF 筛选或出现合理回撤后分批建立。", SellRule: "估值过热或 AI 资本开支预期、光通信订单逻辑转弱时分批减仓。", Note: "AI 基础设施卫星仓。"},
			{Key: "china-internet-etf", Name: "中概互联网 ETF", TargetWeightBP: 0, InstrumentCodes: []string{"513050"}, BuyRule: "当前不配置；若未来增加，从腾讯目标额度中调整。", SellRule: "当前无配置，不以价格波动建立仓位。", Note: "暂定 0%。"},
